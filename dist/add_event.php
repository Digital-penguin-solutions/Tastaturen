...
?> <!DOCTYPE html> <?php include "partials/head.php";?> <html lang="swe"><head><title>Tastaturen add event</title><meta name="description" content="Tastaturen - Add event"></head> <?php 
...
// When an event is going to be viewed for editing
    ...
    ?> <section class="admin_page"><div class="container-fluid full_height"><div class="row full_height"><div class="col-md-8 col-md-offset-2"><h1 class="admin_header">Add event</h1><form id="form" class="add_product_form" method="post" action="add_event.php" enctype="multipart/form-data"><input type="hidden" name="add"> <?php
                        if (isset($_GET["event_id"])){
                            ?> <input type="hidden" value="<?php echo $_GET['event_id'] ?>" name="event_id"> <?php
                        }
                        ?> <h1>Rubrik (Svenska)</h1><textarea name="header" class="short_description"><?php echo $header?></textarea><h1>Rubrik (Danska)</h1><textarea name="header_dk" class="short_description"><?php echo $header_dk?></textarea><h1>Beskrivning (Svenska)</h1><textarea name="info" class="long_description"><?php echo $info?></textarea><h1>Beskrivning (Danska)</h1><textarea name="info_dk" class="long_description"><?php echo $info_dk?></textarea><h1>Datum (Svenska)</h1><input value="<?php echo $date ?>" type="text" name="date"><h1>Datum (Danska)</h1><input value="<?php echo $date_dk ?>" type="text" name="date_dk"><h1>Bild</h1><div class="image_select_container"><p class="center_vertically_css"><strong>New image:</strong></p><input name="main_image" class="center_vertically_css" type="file" onchange="compress_image(event)"><p class="center_vertically_css"><strong>Current:</strong></p><img class="center_vertically_css list_preview_image" src="data:image/jpeg;base64,<?php echo base64_encode( $main_image); ?>" alt="Current image"></div><section class="col-md-12 admin_btn"><button class="col-md-5" id="js-trigger-overlay" onclick="send_form(this, 'add_event.php', 'form')" type="button">Save product</button> <a class="col-md-5 col-md-offset-2" href="admin.php">Cancel edit</a></section></form></div></div></div></section></body> <?php
}
